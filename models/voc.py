...
class NewVOC(nn.Module):
    ...
    def __init__(
        ...
        self.score_embd = nn.Sequential(
                    FFNLayer(
                        d_model=config['input_dim'] * config['num_queries'],
                        dim_feedforward=config['dim_feedforward'],
                        dropout=0.1,
                        normalize_before=pre_norm,
                    ),
                    nn.Linear(config['input_dim'] * config['num_queries'], 1),
                    nn.Sigmoid()
                    )
        self.diff_embd = nn.Sequential(
                    FFNLayer(
                        d_model=config['input_dim'] * config['num_queries'],
                        dim_feedforward=config['dim_feedforward'],
                        dropout=0.1,
        ...
        self.video_query_embed = nn.Embedding(self.num_queries, config['input_dim']) 
    ...
    def forward(self, frame_query, language_query):
        ...
        # Initial
        # frame_score = self.score_embd(frame_query.transpose(1, 2).flatten(2))   # (T, L*B, 1)

        # Modified
        # Target Visualibity Score
        frame_score_vis = self.score_embd(frame_query.transpose(1, 2).flatten(2))   # (T, L*B, 1)
        # Language Matching Score
        lang_match = repeat(language_query, 'b c -> b l t c', b=B, t=T, l =L).flatten(0, 2).unsqueeze(2) # (BLT, C, 1)
        frame_query_match = frame_query.transpose(1, 2).flatten(0, 1) # (BLT, Nq, C)
        lang_match = lang_match / lang_match.norm(dim=1, p=2, keepdim=True)
        frame_query_match = frame_query_match / frame_query_match.norm(dim=-1, p=2, keepdim=True)
        frame_score_match = torch.matmul(frame_query_match, lang_match).mean(dim=1)
        frame_score_match = rearrange(frame_score_match, '(b l t) c -> t (l b) c', t=T, b=B, l=L)
        frame_score_match = torch.sigmoid(frame_score_match)
        # Variance Score
        cum_frame_query = torch.cumsum(frame_query.transpose(1, 2).flatten(2), dim=0)   # (T, L*B, N*C)
        time_idx = torch.arange(1, len(frame_query)+1, dtype=torch.float32).reshape(-1, 1, 1).cuda()    # (T, 1, 1)
        avg_frame_query = cum_frame_query / time_idx    # (T, L*B, N*C)
        diff_frame_query = frame_query.transpose(1, 2).flatten(2) - avg_frame_query # (T, L*B, N*C)
        frame_score_diff = self.diff_embd(diff_frame_query)
        frame_score = frame_score_vis + frame_score_match + 0.5 * frame_score_diff

        frame_query = self.encode_frame_query(frame_query, enc_mask, frame_score) # (T, Nq, L*B, C)
        ...
        language_query = language_query.unsqueeze(1).unsqueeze(1) #[B, 1, 1 C]
        video_query_embeds = self.video_query_embed.weight.unsqueeze(0).unsqueeze(1) #[num_queries, C] -> (1, 1, n, c)
        output = language_query.repeat(1, L, self.num_queries, 1) #[B, L, NQ, C]
        output = output + video_query_embeds
    ...
    def encode_frame_query(self, frame_query, attn_mask, frame_score):
            """
            ...
            frame_score = frame_score.unsqueeze(1).repeat(1, frame_query.shape[1], 1, 1).flatten(0, 1) # (T*Nq, L*B, 1)
            if self.window_size == 0:
                return_shape = frame_query.shape        # T, fQ, LB, C
                frame_query = frame_query.flatten(0, 1) # TfQ, LB, C
                scored_frame_query = frame_query * frame_score

                for i in range(self.enc_layers):
                    frame_query = self.enc_self_attn[i](frame_query, scored_frame_query)
