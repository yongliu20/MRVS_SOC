...
from models.voc import VOC, NewVOC, CrossAttentionLayer
...
class SOC(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.voc = NewVOC(config.VOC)

        self.vlf = MMF(d_model=d_model, nhead=8)
        self.lvf = MMF(d_model=d_model, nhead=8)

        if config.use_proxymmf:
            self.proxy_embed = nn.Embedding(config.num_proxy_query, d_model)
            self.use_proxymmf = True
            self.v2p = MMF(d_model=d_model, nhead=8)
        else:
            self.use_proxymmf = False
        ...
        self.query_fusion = CrossAttentionLayer(
                        d_model=d_model,
                        nhead=8,
                        dropout=0.1
                    )

    def BMRW(self, x, y, w=0.5):
    ...
    def forward(self, samples: NestedTensor, valid_indices, text_queries, targets):
        ...
        for l, (feat, pos_l) in enumerate(zip(backbone_out[-3:], pos[-3:])): 
            ...
            if self.use_proxymmf:
                if proxy_embeds is None:
                    proxy_embeds = self.proxy_embed.weight.unsqueeze(1).repeat(1, B, 1)    # (L, B, C)
                else:
                    proxy_embeds = proxy_embeds
                new_proxy_embeds = self.v2p(tgt=proxy_embeds,
                                memory=src_proj_l_new,
                                memory_key_padding_mask = mask_l,
                                #  tgt_padding_mask = text_word_masks,
                                pos = src_proj_l_new,
                                query_pos = None
                                )# src: [(t h w) b c] lan [l b c]
                proxy_embeds = proxy_embeds + new_proxy_embeds
        ...
        pooled_proxy_embeds = torch.mean(proxy_embeds, dim=0) # (b, c)
        pooled_proxy_embeds = pooled_proxy_embeds.unsqueeze(1).unsqueeze(1)

        tgt = torch.zeros_like(query_embeds)
        tgt = repeat(tgt, 'nq c -> b t nq c', b=B, t=T)
        tgt = tgt + pooled_proxy_embeds
        ...
        if tgt.shape[1] > 64:
            long_time = True
        if long_time:
            tgt_split = list(torch.split(tgt, 64, dim=1))
            len_split = len(tgt_split)
            srcs_split = []
            pose_split= []
            masks_split = []
            for i_src in srcs:
                i_src_split = list(torch.split(i_src, 64, dim=0))
                srcs_split.append(i_src_split)
            srcs_split_final = [list(isrc) for isrc in zip(*srcs_split)]

            for p_src in poses:
                p_src_split = list(torch.split(p_src, 64, dim=0))
                pose_split.append(p_src_split)
            pose_split_final = [list(psrc) for psrc in zip(*pose_split)]

            for m_src in masks:
                m_src_split = list(torch.split(m_src, 64, dim=0))
                masks_split.append(m_src_split)
            masks_split_final = [list(msrc) for msrc in zip(*masks_split)]

            hs, memory, init_reference, inter_references = [], [], [], []
            for isrc in range(len(srcs_split_final)):
                hs_i, memory_i, init_reference_i, inter_references_i, _, _, _ = self.transformer(srcs_split_final[isrc], tgt_split[isrc], masks_split_final[isrc], pose_split_final[isrc], query_embeds)
                hs.append(hs_i)
                memory.append(memory_i)
                init_reference.append(init_reference_i)
                inter_references.append(inter_references_i)
            hs = torch.cat(hs, dim=1)
            memory_reorganize = [list(ihs) for ihs in zip(*memory)]
            memory = []
            for mf in memory_reorganize:
                mf = torch.cat(mf, dim=0)
                memory.append(mf)
            init_reference = torch.cat(init_reference, dim=0)
        ...
        if self.vl_loss:
            ...
            for obj in lang:
                ...
                tempt.append(obj)   
            text_features.append(torch.stack(tempt, dim=0)) # a list of [1 C]
            # tempt = []
        ...
        voc_hs = self.voc(hs, text_features) # VOC_hs [L B NQ C]
        ...
        hs = rearrange(hs, 'l t b q c -> (l b) (t q) c') 
        hs_voc = rearrange(voc_hs, 'l b n c -> (l b) n  c')
        hs_voc = self.BMRW(hs_voc, hs)  # (l b) n  c
        hs_voc = rearrange(hs_voc, 'a b c -> b a c')
        hs = rearrange(hs, 'a b c -> b a c') 
        hs_voc = self.query_fusion(hs, hs_voc)  # (nt, lb, c)
        hs_voc = rearrange(hs_voc, '(t q) (l b) c -> (l b t) c q', t=T, b=B) 
        hs_voc = rearrange(hs_voc, '(l b t) c q -> l t b q c', t=T, b=B)
